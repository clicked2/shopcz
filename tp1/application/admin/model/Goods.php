...
class Goods extends Model
{
	...
	public function imageDelete() {
		//删除图片
		$logoPath = $this->logo;
		$data = substr($logoPath, 0, 9);
		$path = substr($logoPath, 9, strlen($logoPath));
		unlink('static/uploads/'.$logoPath);
		unlink('static/uploads/'.$data.'/sm_'.$path);
		unlink('static/uploads/'.$data.'/mid_'.$path);
		unlink('static/uploads/'.$data.'/big_'.$path);
		unlink('static/uploads/'.$data.'/mbig_'.$path);
	}
}