...
function addToCartShowDiv(goodsId, script_name,goods_recommend,parentId)
{
  ...
  if($(".goods_info_attr").length > 0){
	  if(attr_list == is_selected){
		  Ajax.call(cartUrl, 'goods=' + $.toJSON(goods), addToCartShowDivResponse, 'POST', 'JSON');
	  }else{
...
{
 if ( result.login ) {
    pbDialog('添加到购物车成功',"",0,500,"80",50);
 } else {
    pbDialog('用户未登录 暂时添加到临时购物车',"",0,500,"80",50);
 }
...
function addToCart(goodsId, jump, parentId,event,obj,divId,store_id,end_time,store_mobile)
{
	...
	// goods.spec     = spec_arr;
	goods.goods_id = goodsId;
	goods.cart_number   = number;
  goods.price = $('.m-price').text().substr(1, $('.shop-price').text().length)
  goods.product = $('#goods_attr_num').text()
  if (  $('.hover').length == 2 ) {
    goods.attrs = []
    goods.attrs.push( {attr_id: $('.cartAttr').eq(0).attr('own-data'), goods_attr_id: $('.hover').eq(0).attr('own-data')});
    goods.attrs.push( {attr_id: $('.cartAttr').eq(1).attr('own-data'), goods_attr_id: $('.hover').eq(1).attr('own-data')});
    goods.price = parseFloat(goods.price)  + parseFloat($('.hover').eq(0).attr('own-price')) + parseFloat($('.hover').eq(1).attr('own-price'))
    ...
    console.log(jump)
		//Ajax.call(cartUrl, 'goods=' + $.toJSON(goods), addToCartResponse, 'POST', 'JSON');
    if ( jump ) {
        Ajax.call(cartUrl, 'goods=' + $.toJSON(goods), addToCartResponse, 'POST', 'JSON');
      } else {
         Ajax.call(cartUrl, 'goods=' + $.toJSON(goods), addToCartShowDivResponse, 'POST', 'JSON');
...
function addToCartResponse(result)
{
  ...
  window.location.href = __ROOT__+'index/flow'
