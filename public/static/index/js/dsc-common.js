...
$(function(){
	...
	$("*[ectype='cateItem']").on('mouseenter',function(){
		...
		if(eveval != 1){
			T.data('eveval', '1');
			/*加载中by wu*/
			layer.find("*[ectype='subitems_" + cat_id + "']").html('<img src="'+loadGoods+'" width="200" height="200" class="lazy">');
			$.ajax({
			   type: "GET",
			   url: getCateInfo,
			   data: 'id='+cat_id,
	...
	doc.on('click',"*[ectype='changeBrand']",function(){
		axios.get('./index/index/brands').then(resolve => {
			let str = '<ul>';
			for (let r of resolve.data) {
				str += `
						<li>
			                <div class="brand-img"><a href="#" target="_blank"><img src="${__STATIC__}/uploads/${r.brand_img}"></a></div>
			                <div class="brand-mash">
			                    <div data-bid="204" ectype="coll_brand"></div>
			                </div>
			            </li>
						`
			}
	...
	$("#go_pay").click(function(){
		...
		$.notLogin(login_content, back_url);
		return false;
	});
...
jQuery.notLogin = function(actUrl,backUrl){
	...
	Ajax.call(actUrl,'back_act='+ backUrl, function(data){
		if ( data.login ) {
			//已登录跳转到收货地址页面
			let arr = []
			for (var i = 0; i < $('.item-single').length; i++) {
				if ( $('.item-single').eq(i).children().first().hasClass('selected') ) {
					//goods Id
					let goodsId = $('.item-single').eq(i).find('.cart-checkbox input').val()
					//cart number
					let cartNumber = $('.item-single').eq(i).find('.amount-warp input').val()
					//goods name
					//let goodsName = $('.item-single').eq(i).find('.item-msg a').text()
					//goods img
					let goodsImg = $('.item-single').eq(i).find('.p-img a img').attr('src')
					//status msg
					let statusMsg = $('.item-single').eq(i).find('.tc').text()
					//sum price
					let sumPrice = $('.item-single').eq(i).find('.s-sum font').attr('own-data')
					// shop price
					let shopPrice = $('.item-single').eq(i).find('.s-price strong').attr('own-data')
					arr.push({goodsId, cartNumber, goodsImg, statusMsg, sumPrice, shopPrice})
				}
			}
			if ( arr.length > 0 ) {
				axios.get(getflow2goods, {params: arr}).then(resolve => {
					console.log(resolve)
					if ( resolve.data.msg == 'ok' ) window.location.href = window.location.href+'/flow2'
				})
			}
		} else {
			pb({
...
//提示弹框
function pbDialog(msgTitle,msg,state,width,height,left,cBtn = true, onOk,ok_title,cl_title){
	//msgTitle 主提示信息
	//msg 次标题信息
